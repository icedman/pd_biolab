...
anim_def_t *anim_def_with_len(image_t *sheet, vec2i_t frame_size,
                              float frame_time, uint16_t *sequence,
  ...
  anim_def_t *def =
      malloc(sizeof(anim_def_t) + sizeof(uint16_t) * sequence_len);
  def->sheet = sheet;
...
void anim_rewind(anim_t *anim) { anim->start_time = engine.time; }
...
void anim_draw(anim_t *anim, vec2_t pos) {
  ...
  if (pos.x > rs.x || pos.y > rs.y || pos.x + def->frame_size.x < 0 ||
      pos.y + def->frame_size.y < 0 || anim->color.a <= 0) {
    return;
  }
  ...
  if (anim->rotation == 0) {
    image_draw_tile_ex(def->sheet, tile, def->frame_size, pos, anim->flip_x,
                       anim->flip_y, anim->color);
  } else {
    render_push();
    render_translate(vec2_add(pos, def->pivot));
    render_rotate(anim->rotation);
    image_draw_tile_ex(def->sheet, tile, def->frame_size,
                       vec2_mulf(def->pivot, -1), anim->flip_x, anim->flip_y,
                       anim->color);
    render_pop();
  }
}
