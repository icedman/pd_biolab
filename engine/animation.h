...
#define anim_def(SHEET, FRAME_SIZE, FRAME_TIME, ...)                           \
  anim_def_with_len(SHEET, FRAME_SIZE, FRAME_TIME, (uint16_t[])__VA_ARGS__,    \
                    len((uint16_t[])__VA_ARGS__))

// Create an anim_def with the given sheet, frame_size, sequence and
// sequence_len
...
#define anim(ANIM_DEF)                                                         \
  (anim_t) { .def = ANIM_DEF, .color = rgba_white(), .start_time = engine.time }
