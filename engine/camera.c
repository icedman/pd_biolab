...
static vec2_t camera_viewport_target(camera_t *cam) {
  ...
  vec2_t viewport_target =
      vec2_add(vec2_sub(cam->pos, screen_center), cam->offset);

  if (engine.collision_map) {
    vec2_t bounds = vec2_from_vec2i(vec2i_muli(
        engine.collision_map->size, engine.collision_map->tile_size));
...
void camera_update(camera_t *cam) {
  ...
  if (follow) {
    vec2_t size = vec2(min(follow->size.x, cam->deadzone.x),
    ...
    vec2_t deadzone_target =
        vec2_add(cam->deadzone_pos, vec2_mulf(cam->deadzone, 0.5));
  ...
  if (fabsf(cam->vel.x) + fabsf(cam->vel.y) > cam->min_vel) {
    engine.viewport =
        vec2_add(engine.viewport, vec2_mulf(cam->vel, engine.tick));
  }
}
...
void camera_move(camera_t *cam, vec2_t pos) { cam->pos = pos; }
...
void camera_unfollow(camera_t *cam) { cam->follow = entity_ref_none(); }