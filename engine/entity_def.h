...
#define ENTITY_DEFINE(...)                                                     \
  typedef enum {                                                               \
    ENTITY_TYPE_NONE = 0,                                                      \
    ENTITY_TYPES(ENTITY_DECLARE_TYPE_ENUM) ENTITY_TYPES_COUNT                  \
  } entity_type_t;                                                             \
                                                                               \
  struct entity_t {                                                            \
    ...
    bool                                                                       \
        on_ground; /* True for engine.gravity > 0 and standing on something */ \
    int32_t draw_order; /* Entities are sorted (ascending) by this before      \
                           drawing */                                          \
    ...
    vec2_t pos;   /* Top left position of the bounding box in the game world;  \
                     usually not manipulated directly */                       \
    ...
    float health;      /* When entity is damaged an resulting health < 0, the  \
                          entity is killed */                                  \
    ...
    float max_ground_normal; /* For slopes, determines how steep the slope can \
                                be to set on_ground flag. Default              \
                                cosf(to_radians(46)) */                        \
    float min_slide_normal;  /* For slopes, determines how steep the slope has \
                                to be for entity to slide down. Default        \
                                cosf(to_radians(0)) */                         \
...
#define EDITOR_RESIZE(                                                         \
    RESIZE) // Whether the entity is resizable in the editor. Default false
#define EDITOR_COLOR(                                                          \
    R, G, B) // The box color in the editor. Default (128, 255, 128)
