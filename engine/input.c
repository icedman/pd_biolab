...
void input_init(void) { input_unbind_all(); }
...
void input_set_button_state(button_t button, float state) {
  error_if(button < 0 || button >= INPUT_BUTTON_MAX, "Invalid input button %d",
           button);
...
void input_bind(button_t button, uint8_t action) {
  error_if(button < 0 || button >= INPUT_BUTTON_MAX, "Invalid input button %d",
           button);
  error_if(action < 0 || action >= INPUT_ACTION_MAX, "Invalid input action %d",
           action);
...
vec2_t input_mouse_pos(void) { return vec2(mouse_x, mouse_y); }
...
const char *input_button_to_name(button_t button) {
  if (button < 0 || button >= INPUT_BUTTON_MAX || !button_names[button]) {
    return NULL;
  }
