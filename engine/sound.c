...
void sound_init(int samplerate) { inv_out_samplerate = 1.0 / samplerate; }
...
sound_mark_t sound_mark(void) { return (sound_mark_t){.index = sources_len}; }
...
float sound_global_volume(void) { return global_volume; }
...
void sound_mix_stereo(float *dest_samples, uint32_t dest_len) {
  ...
  for (uint32_t n = 0; n < SOUND_MAX_NODES; n++) {
    ...
    if (node->is_playing && node->volume > 0) {
      ...
      float vol_left =
          volume_normalize * node->volume * clamp(1.0 - node->pan, 0, 1);
      float vol_right =
          volume_normalize * node->volume * clamp(1.0 + node->pan, 0, 1);
      ...
      for (uint32_t di = 0; di < dest_len; di += 2) {
        ...
        dest_samples[di + 1] +=
            src_samples[(source_index << c) + c] * vol_right;

        node->sample_pos += pitch;
        if (node->sample_pos >= source->len || node->sample_pos < 0) {
          if (node->is_looping) {
            node->sample_pos = fmod(node->sample_pos, source->len) +
...
sound_source_t *sound_source(char *path) {
  ...
  error_if(sources_len >= SOUND_MAX_SOURCES, "Max sound sources (%d) reached",
           SOUND_MAX_SOURCES);
  ...
  if (total_samples <= SOUND_MAX_UNCOMPRESSED_SAMPLES) {
    ...
    source->pcm_samples = malloc(total_samples * sizeof(int16_t));
    ...
    do {
      ...
      frame_size = qoa_decode_frame(data + read_pos, file_size - read_pos,
                                    &desc, sample_ptr, &frame_len);
  ...
  // frame here.
    ...
    uint8_t *bump_data = data + read_pos; // malloc(qoa_data_size);
    // memcpy(bump_data, data + read_pos, qoa_data_size);
    // bump_from_temp(data, read_pos, qoa_data_size);

    source->type = SOUND_TYPE_QOA;
    source->qoa = malloc(sizeof(sound_source_qoa_t));
    source->qoa->desc = desc;
    source->qoa->data = bump_data;
    source->qoa->data_len = qoa_data_size;
    source->qoa->pcm_buffer_start = 0;
    source->qoa->pcm_buffer =
        malloc(desc.channels * QOA_FRAME_LEN * sizeof(int16_t));

    uint32_t frame_len;
    uint32_t frame_size =
        qoa_decode_frame(source->qoa->data, file_size - read_pos, &desc,
                         source->qoa->pcm_buffer, &frame_len);
    error_if(frame_size == 0, "QOA decode error for file %s", path);
  }

  source_paths[sources_len] = malloc(strlen(path) + 1);
...
sound_t sound(sound_source_t *source) {
  ...
  for (int i = 0; i < SOUND_MAX_NODES; i++) {
    if (!sound_nodes[i].is_playing && !sound_nodes[i].is_halted &&
        !sound_nodes[i].id) {
      node = &sound_nodes[i];
      sound.index = i;
      break;
...
static inline sound_node_t *sound_get_node(sound_t sound) {
  if (sound.index < SOUND_MAX_NODES &&
      sound_nodes[sound.index].id == sound.id) {
...
void sound_set_time(sound_t sound, float time) {
  ...
  node->sample_pos =
      clamp(time / node->source->samplerate, 0, node->source->len);
}
