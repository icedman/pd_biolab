...
#define SLOPE_NORMAL(X, Y)                                                     \
  {((Y) / SLOPE_LEN((X), (Y))), (-(X) / SLOPE_LEN((X), (Y)))}
...
static const slope_def_t slope_definitions[] = {
    /*     15 NE */ [5] = SLOPE(0, 1, 1, M, SOLID),
    [6] = SLOPE(0, M, 1, N, SOLID),
    [7] = SLOPE(0, N, 1, 0, SOLID),
    /*     22 NE */[3] = SLOPE(0, 1, 1, H, SOLID),
    [4] = SLOPE(0, H, 1, 0, SOLID),
    /*     45 NE */[2] = SLOPE(0, 1, 1, 0, SOLID),
    /*     67 NE */[10] = SLOPE(H, 1, 1, 0, SOLID),
    [21] = SLOPE(0, 1, H, 0, SOLID),
    /*     75 NE */[32] = SLOPE(M, 1, 1, 0, SOLID),
    [43] = SLOPE(N, 1, M, 0, SOLID),
    [54] = SLOPE(0, 1, N, 0, SOLID),

    /*     15 SE */[27] = SLOPE(0, 0, 1, N, SOLID),
    [28] = SLOPE(0, N, 1, M, SOLID),
    [29] = SLOPE(0, M, 1, 1, SOLID),
    /*     22 SE */[25] = SLOPE(0, 0, 1, H, SOLID),
    [26] = SLOPE(0, H, 1, 1, SOLID),
    /*     45 SE */[24] = SLOPE(0, 0, 1, 1, SOLID),
    /*     67 SE */[11] = SLOPE(0, 0, H, 1, SOLID),
    [22] = SLOPE(H, 0, 1, 1, SOLID),
    /*     75 SE */[33] = SLOPE(0, 0, N, 1, SOLID),
    [44] = SLOPE(N, 0, M, 1, SOLID),
    [55] = SLOPE(M, 0, 1, 1, SOLID),

    /*     15 NW */[16] = SLOPE(1, N, 0, 0, SOLID),
    [17] = SLOPE(1, M, 0, N, SOLID),
    [18] = SLOPE(1, 1, 0, M, SOLID),
    /*     22 NW */[14] = SLOPE(1, H, 0, 0, SOLID),
    [15] = SLOPE(1, 1, 0, H, SOLID),
    /*     45 NW */[13] = SLOPE(1, 1, 0, 0, SOLID),
    /*     67 NW */[8] = SLOPE(H, 1, 0, 0, SOLID),
    [19] = SLOPE(1, 1, H, 0, SOLID),
    /*     75 NW */[30] = SLOPE(N, 1, 0, 0, SOLID),
    [41] = SLOPE(M, 1, N, 0, SOLID),
    [52] = SLOPE(1, 1, M, 0, SOLID),

    /*     15 SW */[38] = SLOPE(1, M, 0, 1, SOLID),
    [39] = SLOPE(1, N, 0, M, SOLID),
    [40] = SLOPE(1, 0, 0, N, SOLID),
    /*     22 SW */[36] = SLOPE(1, H, 0, 1, SOLID),
    [37] = SLOPE(1, 0, 0, H, SOLID),
    /*     45 SW */[35] = SLOPE(1, 0, 0, 1, SOLID),
    /*     67 SW */[9] = SLOPE(1, 0, H, 1, SOLID),
    [20] = SLOPE(H, 0, 0, 1, SOLID),
    /*     75 SW */[31] = SLOPE(1, 0, M, 1, SOLID),
    [42] = SLOPE(M, 0, N, 1, SOLID),
    [53] = SLOPE(N, 0, 0, 1, SOLID),
...
static inline void check_tile(map_t *map, vec2_t pos, vec2_t vel, vec2_t size,
                              vec2i_t tile_pos, trace_t *res);
static void resolve_full_tile(map_t *map, vec2_t pos, vec2_t vel, vec2_t size,
                              vec2i_t tile_pos, trace_t *res);
static void resolve_sloped_tile(map_t *map, vec2_t pos, vec2_t vel, vec2_t size,
                                vec2i_t tile_pos, uint32_t tile, trace_t *res);

trace_t trace(map_t *map, vec2_t from, vec2_t vel, vec2_t size) {
  vec2_t to = vec2_add(from, vel);

  trace_t res = {.tile = 0, .pos = to, .normal = vec2(0, 0), .length = 1};
  ...
  vec2_t offset = vec2(vel.x > 0 ? 1 : 0, vel.y > 0 ? 1 : 0);
  ...
  for (int i = 0; i <= steps; i++) {
    vec2i_t tile_pos = vec2i_from_vec2(
        vec2_divf(vec2_add(corner, vec2_mulf(step_size, i)), map->tile_size));
    ...
    if (last_tile_pos.x != tile_pos.x) {
      ...
      if (i > 0) {
        max_y += (vel.y / vel.x) *
                 ((tile_pos.x + 1 - offset.x) * map->tile_size - corner.x);
      }

      int num_tiles =
          ceilf(fabsf(max_y / map->tile_size - tile_pos.y - offset.y));
      for (int t = 0; t < num_tiles; t++) {
        check_tile(map, from, vel, size,
                   vec2i(tile_pos.x, tile_pos.y + dir.y * t), &res);
      }
    ...
    if (last_tile_pos.y != tile_pos.y) {
      ...
      if (i > 0) {
        max_x += (vel.x / vel.y) *
                 ((tile_pos.y + 1 - offset.y) * map->tile_size - corner.y);
      }

      int num_tiles =
          ceilf(fabsf(max_x / map->tile_size - tile_pos.x - offset.x));
      for (int t = corner_tile_checked; t < num_tiles; t++) {
        check_tile(map, from, vel, size,
                   vec2i(tile_pos.x + dir.x * t, tile_pos.y), &res);
      }
...
static void resolve_full_tile(map_t *map, vec2_t pos, vec2_t vel, vec2_t size,
  ...
  vec2_t rp = vec2_add(vec2_from_vec2i(vec2i_muli(tile_pos, map->tile_size)),
                       vec2((vel.x > 0 ? -size.x : map->tile_size),
  ...
  float sign =
      (vel.x * (rp.y - pos.y) - vel.y * (rp.x - pos.x)) * vel.x * vel.y;
...
static void resolve_sloped_tile(map_t *map, vec2_t pos, vec2_t vel, vec2_t size,
  ...
  if (determinant < -epsilon) {
    vec2_t corner =
        vec2_add(vec2_sub(local_pos, ss),
    ...
    if (point_at_vel > -epsilon && point_at_vel < 1 + epsilon &&
        point_at_slope > -epsilon && point_at_slope < 1 + epsilon) {
  ...
  if (sign < 0 || vel.y == 0) {
    ...
    if (rp.y >= max.y || rp.y <= min.y || length > res->length ||
        (!slope->solid && sd.y == 0)) {
      return;
    }
    ...
    if (rp.x >= max.x || rp.x <= min.x || length > res->length ||
        (!slope->solid && sd.x == 0)) {
      return;
    }
