...
#define mat3(a, b, c, d, tx, ty)                                               \
  (mat3_t) { a, b, c, d, tx, ty }
...
static inline vec2_t vec2_divf(vec2_t a, float f) {
  return vec2(a.x / f, a.y / f);
}
...
static inline bool vec2_eq(vec2_t a, vec2_t b) {
  return fabsf(a.x - b.x) + fabsf(a.y - b.y) < VEC2_EQ_EPSILON;
}
...
static inline vec2i_t vec2i_muli(vec2i_t a, int f) {
  return vec2i(a.x * f, a.y * f);
}
static inline vec2i_t vec2i_divi(vec2i_t a, int f) {
  return vec2i(a.x / f, a.y / f);
}
...
static inline bool vec2i_eq(vec2i_t a, vec2i_t b) {
  return a.x == b.x && a.y == b.y;
}
...
static inline rgba_t rgba_blend(rgba_t in, rgba_t out) {
  int in_a = 255 - out.a;
  return rgba((in.r * in_a + out.r * out.a) >> 8,
              (in.g * in_a + out.g * out.a) >> 8,
              (in.b * in_a + out.b * out.a) >> 8, 1);
}

static inline rgba_t rgba_mix(rgba_t a, rgba_t b) {
  return rgba((a.r * b.r) >> 8, (a.g * b.g) >> 8, (a.b * b.b) >> 8,
              (a.a * b.a) >> 8);
}
